...
@Component
public class PythonShExecutor extends RNJobExecutor {

	private static final Logger logger = LoggerFactory.getLogger(PythonShExecutor.class);
	...
	public PythonShExecutor(Properties property) {
		super(property);
	}
	...
	public RNResult exec(String originalCmd, RNContext ctx) throws RNBaseException {
		...
		logger.debug("Run python command '" + cmd + "'");
		OutputStream outStream = new ByteArrayOutputStream();
		//sudo chmod +x /tmp/originalCmd &&  
		
		CommandLine cmdLine = CommandLine.parse(getPyExec());
	...
	protected boolean getKerberosTicket() {
		try {
			...
			AuthService.kerberosConfiguration(properties.getProperty("kerberos.kdcurl"),
					properties.getProperty("kerberos.principal"), properties.getProperty("kerberos.domainname"), getPyExec(),
